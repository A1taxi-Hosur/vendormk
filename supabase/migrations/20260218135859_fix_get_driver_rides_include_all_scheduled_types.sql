/*
  # Fix get_driver_rides_by_date to include scheduled rentals and airport rides

  ## Problem
  Scheduled rental bookings complete via rental_trip_completions with a
  scheduled_booking_id set. The previous function only handled scheduled
  completions in outstation_trip_completions and trip_completions, so scheduled
  rental (and airport) rides showed ₹0 commission in the driver breakdown.

  ## Changes
  - Add scheduled booking sections for rental_trip_completions and
    airport_trip_completions
  - All four tables now handle both scheduled and non-scheduled completions
*/
...
BEGIN
  ...
  -- Local rides (non-scheduled)
  SELECT
    'L-' || SUBSTRING(tc.ride_id::text, 1, 8),
    tc.total_fare,
    tc.total_amount_owed,
    tc.pickup_address,
  ...
  -- Outstation rides (non-scheduled)
  SELECT
  ...
  -- Scheduled bookings via outstation_trip_completions
  SELECT
  ...
  -- Scheduled bookings via trip_completions
  SELECT
    'S-' || SUBSTRING(sb.id::text, 1, 8),
  ...
  -- Scheduled bookings via rental_trip_completions
  SELECT
    'S-' || SUBSTRING(sb.id::text, 1, 8),
  ...
  -- Scheduled bookings via airport_trip_completions
  SELECT
    'S-' || SUBSTRING(sb.id::text, 1, 8),
